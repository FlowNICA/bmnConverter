...
RVec<CbmMCTrack> mcTracks( TClonesArray array ){
	RVec<CbmMCTrack> tracks{};
	tracks.reserve( array.GetSize() );
	for( size_t i=0; i<array.GetSize(); ++i ){
		auto track = static_cast<CbmMCTrack*>( array.operator[](i) );
		if((*track).GetPdgCode() != -1717986919) tracks.push_back( *track );
	}
	return tracks;
}
...
vector<vector<float>> stsTrackParamFirst(RVec<BmnGlobalTrack> global_tracks, RVec<CbmStsTrack> tracks)
try {
...
vector<vector<float>> stsTrackParamLast(RVec<BmnGlobalTrack> global_tracks, RVec<CbmStsTrack> tracks)
try {
  vector<vector<float>> parameters;
  for (auto& global_track : global_tracks) {
    auto idx = global_track.GetGemTrackIndex();
...
void convertBmn_JAM(string inReco="data/run8/rec.root", std::string fileOut = "out.tree.root")
{
...
// 
  ...
  auto dd=d
  	...
  	.Define("b","BmnMCInfo.fB")
  	.Define("simVtxX","BmnMCInfo.fX")
  	.Define("simVtxY","BmnMCInfo.fY")
  	.Define("simVtxZ","BmnMCInfo.fZ")
  	.Define("MCTracks", mcTracks, {"BmnMCInfo.fMCTracks"} )
  	.Define("simMom",simMomentum,{"MCTracks"})
  	.Define("simPdg", simPdg, {"MCTracks"})
	.Define("simMotherId", simMotherId, {"MCTracks"})
    .Define("evtId","DstEventHeader.fEventId")
    ...
    .Define("trSimIndex", recSimIndex, {"BmnGlobalTrack","MCTracks"})
    ...
    .Define("stsTrackParamFirst", stsTrackParamFirst, { "BmnGlobalTrack", "StsVector" })
    .Define("stsTrackParamLast", stsTrackParamLast, { "BmnGlobalTrack", "StsVector" })
    .Define("trParamFirst", trParamFirst, { "BmnGlobalTrack" })
    .Define("trParamLast", trParamLast, { "BmnGlobalTrack" })
    .Define("trTrackCovMatrix", globalTrackCovMatrix, { "BmnGlobalTrack" })
  ...
  vector<string> definedNames =  dd.GetDefinedColumnNames();
  vector<string> toExclude={ "MCTracks", };
  //vector<string> toExclude={  };
  for( auto exlName : toExclude  ){
  	auto delPos = std::find( definedNames.begin(), definedNames.end(), exlName );
  	if( delPos != definedNames.end() )
		definedNames.erase(delPos);
  } 
  dd.Snapshot("t", fileOut, definedNames);
  std::cout << "Written to the file." << std::endl;
}
